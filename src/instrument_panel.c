...
struct ncpanel *create_instrument_panel(struct ncpanel *canvas)
{
    struct notcurses *nc = ncplane_notcurses(ncpanel_get_plane(canvas));
    ...
    struct ncpanel *instrument_panel = ncpanel_create(NULL, y, x, rows, cols); 
